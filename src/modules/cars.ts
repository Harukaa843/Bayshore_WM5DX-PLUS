...
export default class CarModule extends Module {
    register(app: Application): void {
		...
		app.post('/method/create_car', async (req, res) => {
			...
			// =====================================================
			// Grant rewards on new car registration
			// =====================================================
			const opts = Config.getConfig().gameOptions;
			
			const grantItems = async (category: number, count: number | undefined) => {
				if (count && count > 0) {
					console.log(`Granting ${count} items for category ${category} to car ${car.carId}`);
			...
			// Mapping config keys to categories
			await grantItems(15, opts.grantMeterToNewcar); // Meter
			await grantItems(11, opts.grantNameplateToNewcar); // Nameplate
			await grantItems(1, opts.grantCustomcolorTonewCar); // Custom Color
			await grantItems(2, opts.grantAerofullsetTonewCar); // Aero Full Set
			await grantItems(3, opts.grantAeroTonewCar); // Aero
			await grantItems(4, opts.grantBonnetTonewCar); // Bonnet
			await grantItems(5, opts.grantWingTonewCar); // Wing
			await grantItems(6, opts.grantWheelTonewCar); // Wheel
			await grantItems(7, opts.grantMirrorTonewCar); // Mirror
			await grantItems(8, opts.grantNeonTonewCar); // Neon
			await grantItems(9, opts.grantTrunkTonewCar); // Trunk
			await grantItems(10, opts.grantNumberplateTonewCar); // Numberplate
			await grantItems(12, opts.grantGTWingTonewCar); // GT Wing
			await grantItems(13, opts.grantAuraVSTonewCar); // Aura VS
			await grantItems(14, opts.grantWindowsDecorationTonewCar); // Windows Decoration
			await grantItems(16, opts.grantRivalMarkerTonewCar); // Rival Marker
				await grantItems(17, opts.grantStampTonewCar); // Stamp
				await grantItems(18, opts.grantBGMTonewCar); // BGM
				await grantItems(19, opts.grantStickerTonewCar); // Sticker (Body)
				await grantItems(20, opts.grantStickerTonewCar); // Sticker (Side/Roof)

				// Also support createCarRewards if present
			if (opts.createCarRewards && opts.createCarRewards.length > 0) {
				for (let reward of opts.createCarRewards) {
					await prisma.carItem.create({
						data: {
							carId: car.carId,
			...
			// Determine carCoupon type
			let giftCarsTicket = opts.giftCarsTicketToNewUsers;
			let carCouponType = (giftCarsTicket === 1)
				? wm.wm5.protobuf.CarCreationCoupon.CAR_COUPON_HP600
				: wm.wm5.protobuf.CarCreationCoupon.CAR_COUPON_NONE;
