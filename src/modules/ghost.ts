...
import * as ghost_default_car from "./ghost/ghost_util/ghost_default_car";

// Define missing interfaces to satisfy TypeScript
declare module "../wmmt/wm5.proto" {
    export namespace wm5 {
        export namespace protobuf {
            interface SearchCarsByLevelRequest {
                ghostLevel: number;
                area: number;
                regionId?: number;
            }
            interface SearchCarsByLevelResponse {
                error: number;
                ramp: number;
                path: number;
                ghosts: any[];
            ...
            interface SearchCarsRequest {
                area: number;
                selectionMethod: number;
                selectManufacturer?: number;
            }
            ...
            interface LoadGhostDriveDataRequest {
                path: number;
                carTunings: any[];
            }
            namespace LoadGhostDriveDataResponse {
                interface GhostDriveData {
                    carId: number;
                    type: number;
                    driveData: any;
                    trendBinaryByArea: any;
                    trendBinaryByCar: any;
                    trendBinaryByUser: any;
                }
            }
            interface LoadGhostDriveDataResponse {
                error: number;
                data: any[];
            }
            interface LoadPathsAndTuningsRequest {
                selectedCars: number[];
            }
            namespace LoadPathsAndTuningsResponse {
                interface CarTuningsByPath {
                    area: number;
                    ramp: number;
                    path: number;
                    carTunings: any[];
            ...
            interface LoadPathsAndTuningsResponse {
                error: number;
                data: any[];
            }
...
export default class GhostModule extends Module {
    register(app: Application): void {
        ...
        app.post('/method/load_ghost_battle_info', async (req, res) => {
			...
			let carsHistory: wm.wm5.protobuf.Car[] = [];
			if(findChallenger.length > 0)
			{
				for(let i=0; i<findChallenger.length; i++)
				{
					...
					carsHistory.push(wm.wm5.protobuf.Car.create({
						...car!
					}))
				}
			}

			let carsStamp: wm.wm5.protobuf.StampTargetCar[] = [];
			let carsChallenger: wm.wm5.protobuf.ChallengerCar[] = [];
			...
			if(stampTargets)
			{
				for(let i=0; i<stampTargets.length; i++)
				{
					...
					carsStamp.push(
						wm.wm5.protobuf.StampTargetCar.create({
							car: carTarget!,
			...
			if(challengers)
			{
				for(let i=0; i<challengers.length; i++)
				{
					...
					carsChallenger.push(
						wm.wm5.protobuf.ChallengerCar.create({
							ghostCar: wm.wm5.protobuf.GhostCar.create({ car: carTarget! }),
		...
		app.post('/method/load_stamp_target', async (req, res) => {
            ...
            // Get the request body for the load stamp target request
			let body = wm.wm5.protobuf.LoadStampTargetRequest.decode(req.body); 
			...
			// Encode the response
            let message = wm.wm5.protobuf.LoadStampTargetResponse.encode(msg);
        ...
        app.post('/method/search_cars_by_level', async (req, res) => {
            ...
            let body = (wm.wm5.protobuf as any).SearchCarsByLevelRequest.decode(req.body);
			...
			let lists_ghostcar: wm.wm5.protobuf.GhostCar[] = [];
			let arr = [];
			let maxNumber = 0;
			...
			{ 
				...
				if(arr.indexOf(randomNumber) === -1){
					...
					if(!(ghost_trails))
					{ 
						lists_ghostcar.push(wm.wm5.protobuf.GhostCar.create({
			...
			if(body.regionId !== null && body.regionId !== undefined && body.regionId !== 0)
			{
				if(car.length < 1)
				{
					...
					lists_ghostcar.push(wm.wm5.protobuf.GhostCar.create({
						...
						nonhuman: true,
						type: wm.wm5.protobuf.GhostType.GHOST_DEFAULT,
					}))
				}
				// else{} have car list
			}
			...
			let msg = {
				...
				selectionMethod: 2 // GHOST_SELECT_BY_LEVEL
			};

			// Encode the response
            let message = (wm.wm5.protobuf as any).SearchCarsByLevelResponse.encode(msg);
		...
		app.post('/method/search_cars', async (req, res) => {
            ...
            let body = (wm.wm5.protobuf as any).SearchCarsRequest.decode(req.body);
			...
			if(car.length === 0)
			{
				if(body.selectionMethod === 16 ||
				   body.selectionMethod === 17 || 
				   body.selectionMethod === 20)
				{
					// Default car here
			...
			let msg = {
				...
				path: pathVal,
				ghosts: lists_ghostcar,
				selectionMethod: 0 // PATH_NORMAL
			};

			// Encode the response
            let message = (wm.wm5.protobuf as any).SearchCarsResponse.encode(msg);
        ...
        app.post('/method/load_ghost_drive_data', async (req, res) => {
            ...
            // Get the request body for the load ghost drive data request
            let body = (wm.wm5.protobuf as any).LoadGhostDriveDataRequest.decode(req.body);
			...
			{ 
				...
				let ghostType: number = wm.wm5.protobuf.GhostType.GHOST_NORMAL;
				let lists_binarydriveData;
				if(ghost_trails?.driveData !== null && ghost_trails?.driveData !== undefined){
					lists_binarydriveData = (wm.wm5.protobuf as any).BinaryData.create({
				...
				if(ghost_trails?.trendBinaryByArea !== null && ghost_trails?.trendBinaryByArea !== undefined){
					lists_binaryByArea = (wm.wm5.protobuf as any).BinaryData.create({
				...
				if(ghost_trails?.trendBinaryByCar !== null && ghost_trails?.trendBinaryByCar !== undefined){
					lists_binaryByCar = (wm.wm5.protobuf as any).BinaryData.create({
				...
				if(ghost_trails?.trendBinaryByUser !== null && ghost_trails?.trendBinaryByUser !== undefined){
					lists_binaryByUser = (wm.wm5.protobuf as any).BinaryData.create({
                ...
                // Push the data
				lists_ghostcar.push((wm.wm5.protobuf as any).LoadGhostDriveDataResponse.GhostDriveData.create({ // Push the data
            ...
            // Encode the response
			let message = (wm.wm5.protobuf as any).LoadGhostDriveDataResponse.encode(msg);
		...
		app.post('/method/register_ghost_trail', async (req, res) => {
			...
			let body = wm.wm5.protobuf.RegisterGhostTrailRequest.decode(req.body);
			...
			else if(actualSessionId > 0 && actualSessionId < 101)
			{
				...
				// Note: Fields trendBinaryByArea/Car/User might not exist in type, using any to check
                let bodyAny = body as any;
			...
			// Encode the response
            let message = wm.wm5.protobuf.RegisterGhostTrailResponse.encode(msg);
		...
		app.get('/resource/ghost_trail', async (req, res) => {	
			...
			let pTrailId = Number(req.query.trail_id) || undefined;
				...
				{ 
					// Check if this is a crown battle by checking carCrown table
					let isCrown = await prisma.carCrown.findFirst({
						where: {
							carId: pCarId,
							area: pArea
						}
					...
					if (isCrown) {
						...
						// Get the normal ghost trail data
						let ghost_trails = await ghost_trail.getNormalGhostTrail(pCarId, pArea);
		...
		app.post('/method/load_paths_and_tunings', async (req, res) => {
			...
			let body = (wm.wm5.protobuf as any).LoadPathsAndTuningsRequest.decode(req.body);
			...
			for(let j=0; j<19; j++)
			{
				...
				{ 
					...
					{
                        // Push the data
						carTuning.push((wm.wm5.protobuf as any).CarTuning.create({ 
							carId: body.selectedCars[i],
                ...
                // Push the data
				carTbyP.push((wm.wm5.protobuf as any).LoadPathsAndTuningsResponse.CarTuningsByPath.create({
					area: j,
					ramp: rampVal,
					path: pathVal,
            ...
            // Encode the response
			let message = (wm.wm5.protobuf as any).LoadPathsAndTuningsResponse.encode(msg);
        ...
        app.get('/resource/ghost_summary', async (req, res) => {
            ...
            let message = wm.wm5.protobuf.GhostSummary.encode({ ghosts: ghosts });
            common.sendResponse(message, res);
        });
        ...
        app.post('/method/lock_crown', async (req, res) => {
			...
			let body = wmsrv.wm5.protobuf.LockCrownRequest.decode(req.body);
            ...
            // Encode the response
			let message = wmsrv.wm5.protobuf.LockCrownResponse.encode(msg);
