    ...
    register(app: Application): void {
		...
		app.post('/method/load_user', async (req, res) => {
			...
			let msg = {
				...
				userId: user.id,
					...
					// Shop Grade
					shopGrade: user.cars.length > 0 ? user.cars[0].shopGrade : 0,
					unlockAt: user.unlockAt || 0,
		...
		app.post('/method/start_transfer', async (req, res) => {
			...
			app.post('/method/update_user_lock', async (req, res) => {
				...
				// Update user's lock status (unlockAt timestamp)
				if (body.unlockAt !== undefined) {
					await prisma.user.update({
						where: { id: body.userId },
						data: { unlockAt: body.unlockAt }
					});
					console.log(`User ${body.userId} unlockAt updated to ${body.unlockAt}`);
				}
