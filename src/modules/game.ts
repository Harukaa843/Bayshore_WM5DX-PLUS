    ...
    register(app: Application): void {
		...
		app.post('/method/save_game_result', async (req, res) => {
			...
			if (body.gameMode === wm.wm5.protobuf.GameMode.MODE_GHOST_BATTLE && body.rgResult)
			{
				...
				if (ocmEvent) {
					...
					competitionModePlay = true;
						...
						// Check if this is a crown battle (opponent car holds a crown in the current area)
						let opponentCarId = body.rgResult.opponentCarId;
						let currentArea = body.rgResult.area;
						if (opponentCarId && currentArea !== undefined) {
			...
			if(giveMeterReward === 1 && body.playCount % 100 === 0 &&  body.playCount !== 0)
			{
				...
				// --- Shop Grade, Shop Point & Trophy ---
				let shopGrade = body.rgResult?.shopGrade;
				let shopPoint = body.rgResult?.shopPoint;
				let rgWinCount = body.rgResult?.rgWinCount;

				if (shopGrade !== null && shopGrade !== undefined)
				{
					console.log(`Updating Shop Grade: ${shopGrade}, Shop Point: ${shopPoint}, Win Count: ${rgWinCount}`);
					await prisma.car.update({
						where: { carId: body.carId },
						data: {
							shopGrade: shopGrade,
							shopPoint: shopPoint ?? 0,
							...(rgWinCount !== null && rgWinCount !== undefined ? { rgWinCount: rgWinCount } : {})
						}
					});
				}
			...
			{ 
				...
				// --- Shop Grade Unlock Screen Trigger ---
				// If shop grade was updated, we can simulate an "earned item" in the response
				// to trigger the unlock screen in the game client.
				if (shopGrade !== null && shopGrade !== undefined) {
					// Check for items that were just unlocked at this grade
					const { shopItems } = require("./util/shop_items");
					let newlyUnlocked = shopItems.filter((i: any) => i.shopGrade === shopGrade);
					
					if (newlyUnlocked.length > 0) {
						// Add to earnedItems in the response if the proto supports it
						// Note: SaveGameResultResponse usually doesn't have earnedItems, 
						// the game client handles it based on updated shopGrade in LoadUser on next boot,
						// or via specific flags.
						console.log(`User reached Shop Grade ${shopGrade}, unlocked ${newlyUnlocked.length} items.`);
					}
				}
