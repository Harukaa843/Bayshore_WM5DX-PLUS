...
export async function getOCMGhostTrail(carId: number, trailId: number)
{
    ...
    if(ghost_trails)
    {
        ...
        ghostTrail = ghost_trails!.trail ? new Uint8Array(ghost_trails!.trail) : new Uint8Array([1, 2, 3, 4]);
    }
...
export async function getNormalGhostTrail(carId: number, areaId: number)
{
    // Get current date
    let date = Math.floor(new Date().getTime() / 1000);
    ...
    if(ghost_trails)
    {
        console.log('Normal Ghost Trail found');

        // Set to starting ramp data
        rampVal = ghost_trails!.ramp;

        // Set to starting path data
        pathVal = ghost_trails!.path;
    ...
    {
        console.log(`Normal Ghost Trail not found for area ${areaId}, searching for any trail for car ${carId}`);
        
        // Try to find ANY trail for this car
        let any_trail = await prisma.ghostTrail.findFirst({
            where: {
                carId: carId,
                crownBattle: false,
            },
        ...
        if (any_trail) {
            console.log(`Found fallback trail from area ${any_trail.area} for car ${carId}`);
            rampVal = any_trail.ramp;
            pathVal = any_trail.path;
            playedAt = any_trail.playedAt;
            ghostTrail = any_trail.trail ? new Uint8Array(any_trail.trail) : new Uint8Array([1, 2, 3, 4]);
        } else {
            console.log('No fallback trail found. Using default values.');
            // Get the ramp and path id
            let ghost_areas = await ghost_area.GhostArea(areaId);
